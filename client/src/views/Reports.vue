...
export default {
  ...
  setup() {
    ...
    // Bug 7: summary stats as computed properties derived from raw data refs
    ...
    // Bug 8: maxRevenue as computed to avoid O(n²) in getBarHeight
    ...
    const loadData = async () => {
      try {
        // Bug 11: clear error before reload
        error.value = null
        loading.value = true

        // Bug 5: use api methods instead of direct axios
        ...
        // Bug 3: only console.error in catch, no console.log anywhere
    ...
    // Bug 2: wire up filter watch (endpoints don't use filters but we stay consistent)
    watch([selectedPeriod, selectedLocation, selectedCategory, selectedStatus], loadData)

    const formatNumber = (num) => {
      // Bug 12: use toLocaleString instead of hand-rolled implementation
      return num.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    }

    const formatMonth = (monthStr) => {
      // Bug 9: use currentLocale for locale-aware month formatting
      const locale = currentLocale.value === 'ja' ? 'ja-JP' : 'en-US'
      const [year, month] = monthStr.split('-')
      return new Date(year, parseInt(month) - 1, 1).toLocaleDateString(locale, { month: 'short', year: 'numeric' })
    }

    const getBarHeight = (revenue) => {
      // Bug 8: use precomputed maxRevenue instead of looping each call
